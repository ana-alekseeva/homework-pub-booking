...
async def run_scenario(real: bool) -> int:
    ...
    task = (
        ...
        "Use exactly ONE subgoal that calls all tools in this order:\n"
        "  1. venue_search(near='Haymarket', party_size=6, budget_max_gbp=800)\n"
        "  2. get_weather(city='edinburgh', date='2026-04-25')\n"
        "  3. calculate_cost(venue_id=<id from step 1>, party_size=6,\n"
        ...
        "       'condition': <from step 2>, 'temperature_c': <from step 2>,\n"
        "       'total_gbp': <from step 3>, 'deposit_required_gbp': <from step 3>\n"
        "     })\n"
        "  5. complete_task(result={'flyer': 'workspace/flyer.html', 'venue': <venue_id>})\n\n"
    ...
    with example_sessions_dir("ex5-edinburgh-research", persist=True) as sessions_root:
        ...
        try:
            ...
            pass
        planner_system = (
            "You are the PLANNER of an always-on agent. Follow the task exactly — "
            "use the number of subgoals the task specifies (usually one).\n\n"
            "Output ONLY a JSON array with this shape:\n"
            '[{"id":"sg_1","description":"...","success_criterion":"...",'
            '"estimated_tool_calls":5,"depends_on":[],"assigned_half":"loop"}]\n\n'
